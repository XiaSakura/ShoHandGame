...
public class Check {
	...
	public int getWiner() {
		...
		// 对牌类型进行分类统计
		ArrayList<ArrayList<rechecking>> temp = new ArrayList<ArrayList<rechecking>>();
		for (int i = 0; i < allCards.size(); i++) {
			temp.add(rechecking(allCards.get(i)));
		}
		int maxval = 0;
		int playerIndex = 0;
		for (int i = 0; i < temp.size(); i++) {
			if (cheackType(allCards.get(i), temp.get(i).size()) > maxval) {
				playerIndex = i;
				maxval = cheackType(allCards.get(i), temp.get(i).size());
			} else if (cheackType(allCards.get(i), temp.get(i).size()) == maxval) {
				switch (maxval) {
				case 1:
					return danZhangWinner();
				case 2:
					return isYiDuiWinner();
				case 3:
					return isLiangDuiWinner();
				case 4:
					return isSanTiaoWinner();
				case 5:
					return isShunZiWinner();
				case 6:
					return tongHuaWinner();
				case 7:
					return manTangHongWinner();
				case 8:
					return SiTiaoWinner();
				case 9:
					return tongHuaWinner();
				case 10:
					return TongHuaDaShunWinner();
				default:
					return 0;
				}
			}
		...
		// return playerIndex;
		// }
		// System.out.println("系统错误，无法判定赢家");
		// return 0;
	}
	...
	private int cheackType(List<Card> card, int amount) {
		switch (amount) {
		case 5:
			if (isTongHua(card) && isShunZi(card) && "A".equals(card)) {
				// 同花大顺
				return 10;
			} else if (isTongHua(card) && isShunZi(card)) {
				// 同花顺
				return 9;
			} else if (isTongHua(card)) {
				// 同花
				return 6;
		...
		case 4:
			return 2;
		case 3:
			ArrayList<rechecking> rechecking = rechecking(card);
			if (rechecking.get(0).amount == 3) {
				// 三条
				return 4;
			} else {
				// 两对
		...
		case 2:
			if (!"".equals(isSiTiao(card))) {
				return 8;
			} else if (!"".equals(isManTangHong(card))) {
				return 7;
			}
		default:
			break;
